...
from MedST.medst.models.backbones import cnn_backbones
from MedST.medst.models.backbones.med import BertModel
from MedST.medst.models.backbones.vits import create_vit
...
class ImageEncoder(nn.Module):
    def __init__(self,
        ...
        if "vit" in model_name:
            ...
            self.global_embed_la = GlobalEmbedding(
                vision_width, hidden_dim, output_dim
            )

            self.local_embed_la = LocalEmbedding(
                vision_width, hidden_dim, output_dim
            )
    ...
    def vit_forward(self, x, view_type=None, series_len=None ,temporal_embed=None):
        return self.model(x, view_type,series_len, temporal_embed, register_blk=11)

    def forward(self, x, view_type=None, series_len=None, temporal_embed=None, get_local=False):
        if "resnet" in self.model_name:
            return self.resnet_forward(x, get_local=get_local)
        elif "vit" in self.model_name:
            img_feat = self.vit_forward(x, view_type, series_len, temporal_embed)
...
class BertEncoder(nn.Module):
    def __init__(self,
        ...
        #     os.path.join(BASE_DIR, "../../emilyalsentzer/Bio_ClinicalBERT/bert_config.json"))
        self.model = BertModel.from_pretrained(
            os.path.join(BASE_DIR, "../../emilyalsentzer/Bio_ClinicalBERT"),
            # config=self.config,
        ...
        if tokenizer:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(os.path.join(BASE_DIR, "../../emilyalsentzer/Bio_ClinicalBERT/"))
    ...
    def forward(self, ids, attn_mask, token_type,series_len=None, get_local=False):
        outputs = self.model(ids, attn_mask, token_type,series_len=series_len,
