...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.num_series: int = 4  
        # self.temporal_embed = nn.Parameter(torch.zeros(1, self.num_series, embed_dim))
        # self.temporal_embed = nn.Embedding(self.num_series, embed_dim)
        # self.single_embed = nn.Parameter(torch.zeros(1, 1, embed_dim))
        # Spatial positional embeddings
        # 0：frontal  1：lateral
        # self.view_embed = nn.Parameter(torch.zeros(2, 1, embed_dim))

        # trunc_normal_(self.temporal_embed, std=0.02)
        # trunc_normal_(self.single_embed, std=0.02)
    ...
    def forward(self, x, view_type=None, series_len=None, temporal_embed=None, register_blk=-1): # view，series
        # fr = x[0], Lt = x[1], maskk = x[2]
        if len(x) == 3:
            fr, Lt, maskk = x
            B = fr.shape[0]
            fr = self.patch_embed(fr)  # 39,196,768
            Lt = self.patch_embed(Lt)
            Lt = Lt + self.pos_embed[:, 1:, :]

            cls_tokens = self.cls_token.expand(B, -1, -1) # B,1,768
            x = torch.cat((cls_tokens, fr), dim=1)
            x = x + self.pos_embed[:, :x.size(1), :]
            x = torch.cat((x, Lt), dim=1)
            L = x.shape[1]
            
            x = self.pos_drop(x)
            if (view_type == None): view_type = "frontal"
            temp = torch.zeros((B, L))
            for i in range(len(maskk)):
                if maskk[i] == 1: temp[i,197:] = 1

            for i, blk in enumerate(self.blocks):
                x = blk(x, view_type, temp.bool(), register_blk == i)
            x = self.norm(x)
            return x
        else: 
            # print("++++++++++++++++++++++++++++++++++++")
