...
class SSLFineTuner(LightningModule):
    ...
    def shared_step(self, batch):
        ...
        # For multi-class
        with torch.no_grad():
            # feats, _ = self.backbone(x,temporal_embed=self.temporal_embed)
            feats, _ = self.backbone(x, view_type="frontal")
    ...
    def num_training_steps(trainer, dm) -> int:
        ...
        effective_batch_size = trainer.accumulate_grad_batches * trainer.num_gpus

        return (dataset_size // effective_batch_size) * trainer.max_epochs


class SSLEvaluator(nn.Module):
    def __init__(self, n_input, n_classes, n_hidden=2048, p=0.1) -> None:
        super().__init__()
